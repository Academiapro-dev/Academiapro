      ...
      <main style={{ paddingTop: 64 }}>
        <section style={{ padding: '60px 6% 48px', textAlign: 'center', background: 'var(--s1)', borderBottom: '1px solid var(--border)' }}>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap', maxWidth: 600, margin: '0 auto' }}>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher une formation..."
            ...
            {filtered.map((f: any) => (
              <div key={f.code} style={{
                ...
                <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: 16 }}>
                  <span style={{ padding: '3px 9px', borderRadius: 8, background: 'rgba(0,230,118,0.1)', fontSize: 10, color: 'var(--green)', fontWeight: 600 }}>{f.cert}</span>
                  {f.cpf && <span style={{ padding: '3px 9px', borderRadius: 8, background: 'var(--gold-pale)', fontSize: 10, color: 'var(--gold)', fontWeight: 600 }}>CPF ✓</span>}
        ...
        <section style={{ padding: '60px 6%', textAlign: 'center', background: 'var(--s1)', borderTop: '1px solid var(--border)' }}>
          ...
          <Link href="/chat?agent=unia" className="btn-gold">🎯 Entretien gratuit avec UNIA</Link>
        </section>
      </main>
    </>
  )

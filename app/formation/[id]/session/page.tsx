...
export default function SessionFormationAudio({ params }: { params: { id: string } }) {
  const formateur = "default";
  ...
  async function envoyerMessage(texte: string) {
    ...
    try {
      const res = await fetch("/api/formation-audio", {
        method: "POST",
    ...
    <div style={{ minHeight: "100vh", background: "#050508", color: "#fff", fontFamily: "Georgia, serif", padding: "30px 20px" }}>
      <div style={{ maxWidth: "700px", margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: "24px" }}>
          <p style={{ color: "#c8a96e", fontSize: "12px", letterSpacing: "2px", margin: "0 0 4px" }}>COACH AUDIO</p>
          <h1 style={{ fontSize: "22px", margin: "0" }}>Posez vos questions a votre formateur</h1>
        </div>
        ...
        <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(200,169,110,0.2)", borderRadius: "12px", minHeight: "400px", maxHeight: "55vh", overflowY: "auto", padding: "20px", marginBottom: "20px" }}>
          {messages.length === 0 && (
            <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "14px", textAlign: "center", marginTop: "40px" }}>
              Ecrivez ou parlez pour poser votre question
            </p>
          )}
          {messages.map((m, i) => (
          ...
          {loading && <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "13px" }}>Votre formateur reflechit...</p>}
          <div ref={chatEndRef} />
        </div>

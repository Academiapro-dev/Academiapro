...
// ---------------------------------------------------------------------------
// LES VALEURS DE LA BASE, EN FRANCAIS — 24/09.
// La fiche affichait « bancaire », « professionnel », « entite » : les codes
// de la base, tels quels, dans un document remis au client.
// ---------------------------------------------------------------------------
const LIBELLES: Record<string, Record<string, string>> = {
  type_compte: {
    bancaire: "Compte bancaire",
    actifs_numeriques: "Compte d'actifs numériques",
    contrat_capitalisation: "Contrat de capitalisation ou placement de même nature",
  },
  caractere: {
  ...
  titulaire: {
    entite: "La société — le membre déclare le compte comme bénéficiaire d'une procuration (cadres 3.2 et 6.2)",
    personne_physique: "Le membre, titulaire en propre (cadre 3.2)",
  },
};

function libelle(champ: string, v: unknown): string {
  const table = LIBELLES[champ] || {};
  const cle = String(v ?? "");
  return table[cle] || fr(v);
}
...
<table>
  ...
  <tr><td class="label">Type de compte</td><td>${libelle("type_compte", c.type_compte)}</td></tr>
  <tr><td class="label">Caractère</td><td>${libelle("caractere", c.caractere)}</td></tr>
  ...
  <tr><td class="label">Titulaire déclaré</td><td>${libelle("titulaire", c.titulaire)}</td></tr>
...
<div class="alerte">
  ...
  fiscaliste. Point à confirmer avant dépôt : la modalité de détention retenue pour
  chaque compte — titulaire en propre, ou bénéficiaire d'une procuration sur le compte
  de la société — telle qu'elle figure dans le tableau ci-dessus et sur le CERFA.
</div>`;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    //
    // ⛔ REPLI SUPPRIME — 24/09. En cas d erreur de lecture par societe, la
    // route retombait sur TOUS les comptes de l organisme : chez un
    // partenaire qui suit plusieurs LLC, la fiche d une societe aurait liste
    // les comptes des autres. Regle du 31/08 : « un repli qui change le
    // perimetre des donnees n est pas une securite, c est une source de faux
    // silencieux ». Une erreur de lecture est desormais une erreur.
    const { data: lus, error: eLect } = await supabase

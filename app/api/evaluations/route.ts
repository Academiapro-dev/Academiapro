...
export async function GET(req: Request) {
  ...
  const r = await fetch(SB_URL + "/rest/v1/evaluations_questions?formation_slug=eq." + encodeURIComponent(f) + "&type=eq." + t + "&order=ordre.asc&select=id,ordre,question,options", { headers: HD, cache: "no-store" });
...
export async function POST(req: Request) {
  ...
  const rq = await fetch(SB_URL + "/rest/v1/evaluations_questions?formation_slug=eq." + encodeURIComponent(f) + "&type=eq." + t + "&order=ordre.asc&select=bonne_reponse", { headers: HD, cache: "no-store" });
  ...
  await fetch(SB_URL + "/rest/v1/evaluations_reponses", { method: "POST", headers: { ...HD, Prefer: "return=minimal" }, body: JSON.stringify({ email, formation_slug: f, type: t, score, total: qs.length, reponses }) });

...
// Genere les posts sociaux, LinkedIn et Facebook, en ALTERNANCE :
// un passage promeut un article de blog, le suivant une formation.
//
...
async function postsFormation(f: any, lien: string, annee: number, nb: number) {
  const prompt = interdictions(annee, nb)
    ...
    + "DOMAINE : " + (f.domaine || "") + "\n"
    + "DUREE : " + (f.duree || "") + "\n"
    + "PRIX : " + (f.prix ? f.prix + " euros" : "non precise") + "\n"

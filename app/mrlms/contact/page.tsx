...
// ══════════════════════════════════════════════════════════════════════════
// LA PAGE DE CONTACT DE MR LMS — 04/09.
//
...
// une plateforme vendue seule. C est exactement ce que la separation des
// produits vise a eviter — le prospect Mr LMS reste chez Mr LMS.
//
// 🚨 `produit: "mrlms"` EST TRANSMIS A LA ROUTE. Le sujet du courriel porte
// alors « [Mr LMS] », et le message s enregistre avec sa marque dans
...
// par-dessus, /contact DOIT figurer dans PAGES_PUBLIQUES_MRLMS de
// components/NavBar.tsx — livre avec ce fichier.
//
// ⚠️ PAS DE TRADUCTION AUTOMATIQUE ICI, contrairement a la page AcadéMIA :
// Mr LMS s adresse a des organismes de formation francais.
//
...
export default function ContactMrLMS() {
  ...
  async function envoyer() {
    ...
    setEnvoi(true);
    try {
      // L ORGANISME EST PLACE DANS LE SUJET. La route attend cinq champs :
      // produit, prenom, nom, email, sujet, message. Le nom de l organisme
      // est ce qui identifie le mieux un prospect Mr LMS, il tient donc lieu
      // de sujet plutot que de demander une ligne de plus.
      const r = await fetch("/api/contact", {
        ...
        body: JSON.stringify({
          ...
          nom: nom,
          email: email,
          sujet: organisme ? "Demande — " + organisme : "Demande de présentation",
          message: message,
        }),
      });
      ...
      <main style={{ maxWidth: "620px", margin: "0 auto",
        ...
        <div style={{ textAlign: "center", marginBottom: "36px" }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.6)", fontSize: "15px",
            ...
            Une heure, sur vos propres cas. Nous parcourons ensemble un
            parcours complet, du premier inscrit au bilan.
          </p>
        </div>
        ...
        <div style={{ background: "#1a1a2e", borderRadius: "16px",
          ...
          {parti ? (
            ...
            <>
              ...
              <div style={{ marginBottom: "20px" }}>
                <label style={labelStyle}>Votre organisme</label>
                <input type="text" style={champStyle} value={organisme}
                  onChange={function (e) { setOrganisme(e.target.value); }} />
              </div>

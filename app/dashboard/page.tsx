...
export default function DashboardPage() {
  ...
  async function envoyer() {
    ...
    try {
      ...
      setChat(p=>[...p,{role:"agent",text:"Erreur."}]);
    }
    setLoading(false);
  }
    ...
    <div style={{backgroundColor:"#050508",minHeight:"100vh",color:"#fff",padding:"30px 20px"}}>
      <div style={{maxWidth:"900px",margin:"0 auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"15px",marginBottom:"35px"}}>
          {[
            {titre:"Formations",valeur:formations.length.toString(),icon:"🎓"},
        ...
        <div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(200,169,110,0.2)",borderRadius:"12px",padding:"20px",minHeight:"280px",marginBottom:"15px",maxHeight:"400px",overflowY:"auto"}}>
          {chat.length===0&&<p style={{color:"rgba(255,255,255,0.3)",textAlign:"center",marginTop:"100px"}}>Posez une question a votre agent tuteur...</p>}
          {chat.map((msg,i)=>(
        ...
        <div style={{display:"flex",gap:"10px",marginBottom:"25px"}}>
          <input type="text" placeholder="Posez votre question..." value={message} onChange={e=>setMessage(e.target.value)} onKeyDown={e=>e.key==="Enter"&&envoyer()} style={{flex:1,padding:"12px",borderRadius:"8px",border:"1px solid rgba(200,169,110,0.3)",background:"rgba(255,255,255,0.05)",color:"#fff"}}/>
          <button onClick={envoyer} disabled={loading} style={{padding:"12px 24px",background:"#c8a96e",color:"#050508",border:"none",borderRadius:"8px",fontWeight:"bold",cursor:"pointer"}}>Envoyer</button>
        </div>
        {reco.length>0&&(
          ...
          <div style={{display:"grid",gap:"12px"}}>
            {formations.map((f:any)=>(
              <div key={f.code} style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(200,169,110,0.2)",borderRadius:"10px",padding:"16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                ...
                <a href={"/formation/"+f.code} style={{background:"#c8a96e",color:"#050508",padding:"8px 16px",borderRadius:"6px",textDecoration:"none",fontSize:"13px",fontWeight:"bold"}}>Acceder</a>
              </div>
            ))}
          </div>
        </div>

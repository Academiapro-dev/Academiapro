    ...
    <div style={{ background: "#050508", minHeight: "100vh", color: "#fff" }}>
      <div style={{ background: "linear-gradient(135deg,#0a0a1a,#1a1a2e)", padding: "30px", borderBottom: "2px solid rgba(200,169,110,0.3)" }}>
        <div style={{ maxWidth: "900px", margin: "0 auto" }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "14px", margin: 0 }}>Expert Finance & Gestion — Comptabilité · Fiscalité · Investissement · France & Israël</p>
        </div>
      </div>
        ...
        <div style={{ display: "flex", gap: "10px", marginBottom: "20px" }}>
          {["france", "israel", "international"].map(c => (
            <button key={c} onClick={() => setContexte(c)}
              style={{ padding: "8px 20px", borderRadius: "8px", border: "none", cursor: "pointer", background: contexte === c ? "#c8a96e" : "rgba(255,255,255,0.05)", color: contexte === c ? "#050508" : "rgba(255,255,255,0.6)", fontWeight: contexte === c ? "bold" : "normal", textTransform: "capitalize" }}>
              {c === "france" ? "🇫🇷 France" : c === "israel" ? "🇮🇱 Israël" : "🌍 International"}
            </button>
          ))}
        </div>

        <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(200,169,110,0.2)", borderRadius: "12px", padding: "25px", minHeight: "500px", display: "flex", flexDirection: "column" }}>
          <div style={{ flex: 1, overflowY: "auto", marginBottom: "15px", maxHeight: "400px" }}>
            {historique.length === 0 && (
              <div style={{ textAlign: "center", paddingTop: "80px" }}>
                <p style={{ color: "rgba(255,255,255,0.3)", fontSize: "16px" }}>Bonjour. Je suis le Professeur Henri Mercier.</p>
                <p style={{ color: "rgba(255,255,255,0.2)", fontSize: "14px" }}>Posez votre question comptable ou financière...</p>
              </div>
            )}
            {historique.map((msg, i) => (
              <div key={i} style={{ marginBottom: "15px", display: "flex", justifyContent: msg.role === "user" ? "flex-end" : "flex-start" }}>
                <div style={{ background: msg.role === "user" ? "#c8a96e" : "rgba(255,255,255,0.08)", color: msg.role === "user" ? "#050508" : "#fff", padding: "12px 16px", borderRadius: "10px", maxWidth: "80%", fontSize: "14px", lineHeight: "1.7", whiteSpace: "pre-wrap" }}>

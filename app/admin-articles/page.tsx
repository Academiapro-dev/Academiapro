...
export default function AdminArticlesPage() {
  ...
  async function agir(action: string, id: string) {
    setChargement(true);
    try {
      await fetch(
  ...
  if (!connecte) {
    return (
      <div style={{ minHeight: "100vh", background: "#050508", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
        <div style={{ maxWidth: "380px", width: "100%", textAlign: "center" }}>
          <h1 style={{ color: OR, fontFamily: "Georgia,serif", marginBottom: "8px" }}>Validation des articles</h1>
          ...
          <button onClick={() => charger(secret)} disabled={chargement}
            style={{ width: "100%", padding: "13px", background: OR, color: "#050508", border: "none", borderRadius: "8px", fontWeight: "bold", cursor: "pointer" }}>
            {chargement ? "..." : "Entrer"}
          </button>
          {message && <p style={{ color: "#ff6b6b", marginTop: "14px", fontSize: "13px" }}>{message}</p>}
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#050508", color: "#fff", padding: "40px 20px" }}>
      <div style={{ maxWidth: "800px", margin: "0 auto" }}>
        <h1 style={{ color: OR, fontFamily: "Georgia,serif", marginBottom: "6px" }}>Brouillons en attente</h1>
        ...
        {brouillons.length === 0 && (
          <p style={{ opacity: 0.5 }}>Aucun brouillon. Le redacteur repassera dimanche.</p>
        )}
        {brouillons.map((a) => (
          <div key={a.id} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(200,169,110,0.3)", borderRadius: "12px", padding: "20px 22px", marginBottom: "16px" }}>
            ...
            {ouvert === a.id && (
              <div style={{ background: "rgba(0,0,0,0.3)", borderRadius: "8px", padding: "16px", marginBottom: "14px", maxHeight: "320px", overflowY: "auto", whiteSpace: "pre-wrap", fontSize: "13px", lineHeight: 1.8, opacity: 0.85 }}>
                {a.contenu}
              </div>
            )}
            <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
              <button onClick={() => setOuvert(ouvert === a.id ? null : a.id)}
                style={{ background: "none", border: "1px solid rgba(200,169,110,0.4)", color: OR, padding: "9px 18px", borderRadius: "8px", cursor: "pointer", fontSize: "13px" }}>
                {ouvert === a.id ? "Fermer" : "Lire"}
              </button>
              <button onClick={() => agir("publier", a.id)} disabled={chargement}
                style={{ background: "#22c55e", border: "none", color: "#050508", padding: "9px 18px", borderRadius: "8px", cursor: "pointer", fontSize: "13px", fontWeight: "bold" }}>
                Publier
              </button>
              <button onClick={() => agir("refuser", a.id)} disabled={chargement}
                style={{ background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.5)", color: "#ef4444", padding: "9px 18px", borderRadius: "8px", cursor: "pointer", fontSize: "13px" }}>
                Refuser
              </button>
            </div>
          </div>

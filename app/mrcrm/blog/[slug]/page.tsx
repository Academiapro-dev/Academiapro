...
// ══════════════════════════════════════════════════════════════════════════
// ARTICLE DU BLOG MR CRM — 04/09.
//
// Ne sert QUE les articles de marque `mrcrm` : un slug d une autre marque
// renvoie 404 ici, meme s il existe en base.
//
// 🚨 CANONIQUE, LIEN DE RETOUR ET BOUTONS SONT ABSOLUS, SUR www.mrcrm.fr.
// Le middleware sert cette page sous mrcrm.fr/blog/<slug> : c est cette
// adresse que le sitemap declare. La page est aussi servie depuis
// academiapro.fr/mrcrm/blog/<slug> ; un lien relatif « Tous les articles »
...
// 🆕 LE CONTACT EST CELUI DE MR CRM — 04/09.
...
export async function generateMetadata({ params }: any) {
  ...
  if (!article) {
    return { title: "Article introuvable — Mr CRM" };
  }
  const url = SITE + "/blog/" + article.slug;
  return {
    title: article.titre + " — Mr CRM",
...
export default async function ArticleMrCRM({ params }: any) {
  const p = await params;
  const article = await lireArticle(p.slug);
  if (!article) notFound();
    ...
    <div style={{ minHeight: "100vh", background: FOND, color: "#fff",
      ...
      <header style={{ borderBottom: "1px solid rgba(200,169,110,0.15)",
        ...
        <div style={{ ...SECTION, display: "flex",
          ...
          {/* 🚨 LE MEME MENU SUR TOUTES LES PAGES — 04/09. Un article lu
              depuis une recherche est souvent la premiere page vue.
              ⚠️ AJOUTER « FONCTIONS » ICI le jour ou les pages de fonction
              de Mr CRM existeront. */}
      ...
      <main style={{ maxWidth: "760px", margin: "0 auto",
        ...
        {/* ---- APPEL ---- Vers la vitrine, dont l existence est certaine.
            Pas vers /contact : sur mrcrm.fr ce chemin serait reecrit vers
            /mrcrm/contact. */}
        <div style={{ marginTop: "50px",
          ...
          <p style={{ color: "rgba(255,255,255,0.6)", fontSize: "15px",
            ...
            Mr CRM range vos contacts, vos échanges et vos relances au même
            endroit : savoir qui rappeler, et quoi lui dire.
          </p>
          <a href={SITE + "/"}
      ...
      {/* ---- PIED ---- Pages legales d AcadéMIA Pro, en absolu, comme sur
          l index : un lien relatif serait reecrit vers /mrcrm/... */}

...
export const metadata = {
  title: "Conditions d'utilisation — Mr CRM",
};
...
// ══════════════════════════════════════════════════════════════════════════
// LES CONDITIONS D UTILISATION DE MR CRM — 14/09.
//
// Meme mecanique que la page de confidentialite : le texte vit en base
// (textes_legaux, cle « cgu_crm »), une correction se fait par SQL sans
// redeploiement.
//
// 🚨 LE DOSSIER S APPELLE « cgv » ET L ADRESSE PUBLIQUE EST /cgv, parce que
// c est celle qui a ete declaree dans la console Google. Le titre, lui, dit
// « conditions d utilisation » : Mr CRM se vend sur devis, pas au detail.
// SI CETTE ADRESSE CESSE DE REPONDRE, la publication de l ecran de
// consentement tombe et les agendas des clients se deconnectent au bout de
// sept jours. On ne la deplace pas, on ne la renomme pas.
//
// ⚠️ ELLE DOIT FIGURER DANS PAGES_PUBLIQUES_MRCRM (components/NavBar.tsx) :
// sans cela, un visiteur non connecte verrait la barre de travail du client
// par-dessus une page publique.
// ══════════════════════════════════════════════════════════════════════════

export default async function ConditionsMrCRM() {
    ...
    <div style={{ minHeight: "100vh", background: NOIR, color: "#fff", fontFamily: "Georgia, serif" }}>
      ...
      <header style={{ borderBottom: "1px solid rgba(200,169,110,0.15)", padding: "22px 0" }}>
        <div style={{ maxWidth: "820px", margin: "0 auto", padding: "0 24px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "12px" }}>
          <a href="/" style={{ color: OR, fontSize: "21px", fontWeight: "bold", textDecoration: "none" }}>
            Mr CRM
          </a>
          <a href="/" style={{ color: "rgba(255,255,255,0.6)", fontSize: "15px", textDecoration: "none" }}>
            ← Retour
          </a>
        </div>
      </header>
      ...
      <div style={{ maxWidth: "820px", margin: "0 auto", padding: "56px 24px 90px" }}>
        ...
        {contenu.length === 0 ? (
          <p style={{ color: "rgba(255,255,255,0.6)", fontSize: "16px" }}>
            ...
            contact@academiapro.fr et nous vous le transmettons.
          </p>
        ) : (
          <>
            {maj && (
      ...
      <footer style={{ borderTop: "1px solid rgba(200,169,110,0.15)", padding: "30px 0" }}>
        <div style={{ maxWidth: "820px", margin: "0 auto", padding: "0 24px" }}>
          <p style={{ color: "rgba(255,255,255,0.45)", fontSize: "14px", lineHeight: "1.8", margin: 0 }}>
            Mr CRM — une marque d&apos;AcadéMIA Pro LLC<br />
          ...
          <p style={{ margin: "16px 0 0", display: "flex", gap: "20px", flexWrap: "wrap" }}>
            <a href="/" style={lienPied}>Accueil</a>
            <a href="/confidentialite" style={lienPied}>Politique de confidentialité</a>
          </p>
        </div>
      </footer>

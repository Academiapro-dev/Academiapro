...
export const metadata = {
  title: "Politique de confidentialité — Mr CRM",
};
...
// ══════════════════════════════════════════════════════════════════════════
// LA CONFIDENTIALITE DE MR CRM — 14/09.
//
// Meme mecanique que les mentions de Mr Comptable : le texte vit en base
// (textes_legaux, cle « confidentialite_crm »), une correction se fait par
// SQL sans redeploiement.
//
// 🚨 CETTE PAGE EST EXIGEE PAR GOOGLE pour publier l ecran de consentement
// de l agenda. Son adresse — https://www.mrcrm.fr/confidentialite — est
// declaree dans la console Google : SI ELLE CESSE DE REPONDRE, la
// publication tombe et les agendas des clients se deconnectent au bout de
...
export default async function ConfidentialiteMrCRM() {
  const { data } = await supabase
    .from("textes_legaux")
    .select("titre, contenu, maj_le")
    .eq("cle", "confidentialite_crm")
    .maybeSingle();
      ...
      <footer style={{ borderTop: "1px solid rgba(200,169,110,0.15)", padding: "30px 0" }}>
        <div style={{ maxWidth: "820px", margin: "0 auto", padding: "0 24px" }}>
          ...
          <p style={{ margin: "16px 0 0", display: "flex", gap: "20px", flexWrap: "wrap" }}>
            ...
            <a href="/cgv" style={lienPied}>Conditions d&apos;utilisation</a>
          </p>
        </div>
      </footer>

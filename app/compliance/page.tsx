...
// ---------------------------------------------------------------------------
// 🚨 CET ECRAN VIT SOUS /compliance ET NON PLUS SOUS /admin — 07/09.
//
// LE DEFAUT. Il etait place sous app/admin/compliance/page.tsx, chemin que
// le middleware reserve a contact@academiapro.fr. AUCUN CLIENT MYSTERLLC NE
// POUVAIT DONC OUVRIR SA PROPRE FICHE : ni voir ses echeances, ni generer
// ses formulaires, ni tenir sa comptabilite. Le produit n avait pas
// d espace client du tout.
//
// 🚨 LA SECURITE N EST PAS DANS LE CHEMIN, ELLE EST DANS LA ROUTE.
// /api/compliance/dashboard lit le tenant dans le COOKIE SIGNE, borne
// toutes ses requetes dessus, et verifie que l entite demandee appartient
// bien a ce tenant. Un client qui ouvre cet ecran ne verra que SES
// societes : la route ne lui renverra rien d autre.
//
// ⚠️ L ECRAN NE CONTIENT AUCUNE DONNEE. Il les demande. Le proteger par le
// chemin n ajoutait donc rien — cela empechait seulement les clients d y
// acceder.
//
// ⚠️ TOUTES LES ROUTES APPELEES ICI SONT DEJA SOUS /api/compliance, hors
// /admin, et font chacune leur propre controle de session. Rien a changer
// de ce cote.
//
// 🚨 /compliance DOIT FIGURER DANS `CHEMINS_ELEVE` DU MIDDLEWARE — le
// groupe des chemins a session simple, comme /organisme. Sans cela, un
// client connecte serait renvoye vers la connexion.
// ---------------------------------------------------------------------------

// ---------------------------------------------------------------------------
// 🚨 LE TABLEAU DE BORD PORTE SUR UNE SOCIETE PRECISE — 31/08.
//

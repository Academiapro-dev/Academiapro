...
export default function PageTVA() {
  ...
  function decaler(pas: number) {
    const base = mois && /^\d{4}-\d{2}$/.test(mois)
      ? mois
      : (d && d.periode ? String(d.periode.debut).slice(0, 7) : "");
    if (!base) return;
    const annee = parseInt(base.slice(0, 4), 10);
    const m = parseInt(base.slice(5, 7), 10);
    const suivant = new Date(Date.UTC(annee, m - 1 + pas, 1));
    setMois(suivant.getUTCFullYear() + "-" + String(suivant.getUTCMonth() + 1).padStart(2, "0"));
  }
  ...
  function Ligne({ nom, montant, sens, fort }: any) {
    return (
      <div style={{ display: "flex", justifyContent: "space-between", padding: "11px 0", borderBottom: "1px solid rgba(255,255,255,0.06)", gap: "12px" }}>
        <span style={{ color: fort ? "#fff" : "rgba(255,255,255,0.72)", fontSize: fort ? "15.5px" : "14.5px", fontWeight: fort ? "bold" : "normal", lineHeight: "1.6" }}>
          {nom}
        </span>
    ...
    <div style={CADRE}>
      <div style={{ maxWidth: "900px", margin: "0 auto" }}>
        ...
        {chargement ? (
          ...
          <div style={CARTE}>
            <p style={{ color: "rgba(255,255,255,0.75)", fontSize: "15px", margin: 0, lineHeight: "1.75" }}>
              {d.note}
            </p>
          </div>
        ) : (
          <>
            {d.formulaire === "CA3" && (
              <div style={{ display: "flex", gap: "12px", alignItems: "center", marginBottom: "18px", flexWrap: "wrap" }}>
                ...
                <span style={{ color: "#c8a96e", fontSize: "17px", fontWeight: "bold" }}>
                  {d.periode.libelle}
                </span>
            ...
            <div style={{ ...CARTE, border: "2px solid " + (d.tva.a_decaisser > 0 ? "rgba(232,163,61,0.5)" : "rgba(76,175,80,0.5)") }}>
              ...
              {d.tva.a_decaisser > 0 ? (
                <>
                  <p style={{ color: "#e8a33d", fontSize: "32px", fontWeight: "bold", margin: "0 0 4px" }}>
                ...
                <>
                  <p style={{ color: "#4caf50", fontSize: "32px", fontWeight: "bold", margin: "0 0 4px" }}>
                    {euros(d.tva.credit_a_reporter)}
                  </p>
                  <p style={{ color: "rgba(255,255,255,0.6)", fontSize: "14px", margin: 0 }}>
                    Credit de TVA a reporter sur la periode suivante
                  </p>
                </>
              )}
            </div>
            ...
            <div style={CARTE}>
              ...
              <p style={{ color: "rgba(255,255,255,0.45)", fontSize: "13px", margin: "14px 0 0", lineHeight: "1.75" }}>
                Bases lues sur la periode : {euros(d.bases.produits_ht)} de produits,
            ...
            {d.tva.a_decaisser > 0 && (
              <div style={{ ...CARTE, border: d.controle.liquidation_passee ? "1px solid rgba(76,175,80,0.4)" : "1px solid rgba(232,163,61,0.45)" }}>
                <p style={{ color: d.controle.liquidation_passee ? "#4caf50" : "#e8a33d", fontSize: "14.5px", margin: 0, lineHeight: "1.8" }}>
                  {d.controle.liquidation_passee
                    ...
                    : "Ecriture de liquidation absente ou incomplete : le compte 445510 porte "
                      + euros(d.controle.solde_compte_445510) + " alors que le calcul donne "
                      + euros(d.tva.a_decaisser) + ". Passez l ecriture avant de declarer."}
                </p>
              </div>
            )}

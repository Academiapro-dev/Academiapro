...
export default function SessionsPage() {
  ...
  useEffect(() => { if (onglet === "fichiers") chargerFichiers(); }, [onglet]);

  async function chargerSessions() {
    setLoading(true);
    const r = await fetch(SUPABASE_URL + "/rest/v1/agent_memories?select=id,agent_id,session_label,created_at&order=created_at.desc&limit=100", {
  ...
  async function chargerFichiers() {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        setFichiers(data
          .filter(f => f.name && f.name !== "test_1782492093.jpg")
          .map(f => ({
            nom: f.name,
  ...
  async function supprimerFichier(nom) {
    ...
    await fetch(SUPABASE_URL + "/storage/v1/object/agent_documents/" + nom, {
      method: "DELETE",
      headers: { "apikey": SUPABASE_KEY, "Authorization": "Bearer " + SUPABASE_KEY }
    });
    chargerFichiers();
  ...
  const fichiersFiltres = fichiers.filter(f => {
    if (filtreType === "pdf") return f.type === "pdf";
    if (filtreType === "image") return f.type === "image";
    return true;
  });
        ...
        {onglet === "fichiers" && (
          <div>
            <div style={{ display: "flex", gap: "10px", marginBottom: "20px", alignItems: "center" }}>
              ...
              <span style={{ color: "rgba(255,255,255,0.4)", fontSize: "13px", marginLeft: "auto" }}>
                {fichiersFiltres.length} fichier(s)
              </span>
              <button onClick={chargerFichiers} style={{ padding: "8px 16px", borderRadius: "8px", border: "1px solid rgba(200,169,110,0.3)", background: "transparent", color: "#c8a96e", cursor: "pointer", fontSize: "13px" }}>
                🔄 Actualiser
              </button>
            </div>

            {loadingFichiers ? (
              <p style={{ color: "rgba(255,255,255,0.4)", textAlign: "center", padding: "50px" }}>Chargement des fichiers...</p>
            ) : fichiersFiltres.length === 0 ? (
              <p style={{ color: "rgba(255,255,255,0.3)", textAlign: "center", padding: "50px" }}>Aucun fichier trouvé</p>
            ) : (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: "15px" }}>
                {fichiersFiltres.map((f, i) => (
                  <div key={i} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(200,169,110,0.2)", borderRadius: "12px", padding: "12px", display: "flex", flexDirection: "column", gap: "10px" }}>
                    ...
                    <div>
                      <div style={{ color: "#c8a96e", fontSize: "11px", fontWeight: "bold", marginBottom: "3px" }}>
                        {EMOJIS[f.agent] || "📋"} {f.agent}
                      </div>
                      <div style={{ color: "rgba(255,255,255,0.3)", fontSize: "10px" }}>{f.date}</div>
                    </div>
                    <div style={{ display: "flex", gap: "6px" }}>
                      <a href={f.url} target="_blank" rel="noopener noreferrer"
                        style={{ flex: 1, textAlign: "center", padding: "8px", background: "#c8a96e", color: "#050508", borderRadius: "6px", textDecoration: "none", fontSize: "12px", fontWeight: "bold" }}>
                        👁 Voir
                      </a>
                      <a href={f.url} download
                        style={{ flex: 1, textAlign: "center", padding: "8px", background: "rgba(200,169,110,0.2)", color: "#c8a96e", borderRadius: "6px", textDecoration: "none", fontSize: "12px", fontWeight: "bold", border: "1px solid rgba(200,169,110,0.3)" }}>
                        ⬇️ DL
                      </a>
                      <button onClick={() => supprimerFichier(f.nom)}
                        style={{ padding: "8px", background: "rgba(255,0,0,0.15)", color: "#ff6b6b", border: "none", borderRadius: "6px", cursor: "pointer", fontSize: "12px" }}>
                        🗑
                      </button>
                    </div>
                  </div>
